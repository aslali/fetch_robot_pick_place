...
class ArmControl(object):
    ...
    def __init__(self):
        ...
        rospy.loginfo("Waiting for fetch_arm controller....")
        self.client.wait_for_server()
        if self.client.wait_for_server():
            rospy.loginfo("Got fetch_arm controller")
        else:
            rospy.logerr(
                "Couldn't connect to fetch_arm controller.... Did you roslaunch fetch_moveit_config move_group.launch?")
        ...
        self.move_group = MoveGroupInterface("fetch_arm", "base_link")
    ...
    def plan_joint_position(self, joint_angles, wait=True):
    ...
    def move_cartesian_position(self, position, orientation=[0.0, 0.0, 0.0], constraints=None, wait=True, v_scale=1, a_scale=1):
        ''' position: xyz
            orientation: rpy'''
        ...
        self.arm_group.set_max_acceleration_scaling_factor(a_scale)
        self.arm_group.set_max_velocity_scaling_factor(v_scale)
    ...
    def intermediate_stow(self, planning=True, wait=True, **kwargs):
        if planning:
            result = self.plan_joint_position(self.inter_stow_joint_value, wait=wait)
        else:
            if kwargs is not None:
                if "max_velocity_scaling_factor" not in kwargs:
                    kwargs["max_velocity_scaling_factor"] = 0.05
...
if __name__ == '__main__':
    ...
    ## hug_values = [1.4, -0.35, 0.0, 0.4, 2.0, -2, -1.5]
    # hug_values = [1.4, 0.25, 0.3, -0.99, 1.3, -1.8, 0.5 + 3.14]
    # hug_joint_value = {}
    # for j, joint in enumerate(arm_module.joint_names):
    #     hug_joint_value[joint] = hug_values[j]
    # arm_module.plan_joint_position(hug_joint_value)
