...
if __name__ == '__main__':
    ...
    markers.daemon = True
    markers.start()
    fetch_server = server.ServerControl()
    fetch_server.daemon = True
    fetch_server.start()
    pick_ids = [1, 2]
    place_ids = {1: 3, 2: 0}
    ...
    while pick_ids and markers.is_alive():
        ...
        if not imd:
            ...
            for i in range(7):
                ...
                print(imd)
                if not imd:
                    fetch_base.move_forward(distance=0.25)
                    time.sleep(0.1)
        ...
        if imd:
            ...
            fetch_base.goto(x=-3, y=0.14, theta=0)   #x=-2.57
            # print(fetch_base.get_pose())
            fetch_base.move_forward(distance=0.4)
            time.sleep(1)
    ...
    fetch_server.server_disconnect()